...
//             li.innerHTML = `<div class="wee"><strong>${task.task}: </strong> ${task.description}</div>
//                 <div class="buttons">
//                     <input type="checkbox" class="complete_checkbox" ${task.status === 'finished' ? 'checked' : ''}>
//                     <button class="update-btn"><i class="fa-regular fa-pen-to-square"></i></button>
...
//         if (window.UpdatingTask) {
//             tasks[window.UpdatingTask] = { task, description, status };
//             window.UpdatingTask = null;
//         } else {
//             const newKey = Date.now().toString();
//             tasks[newKey] = { task, description, status };
//         }
...
//             const li = event.target.closest('li');
//             const key = li.dataset.key;
//             const newStatus = event.target.checked ? 'finished' : 'open';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('task-form').addEventListener('submit', function (event) {
      ...
      if (window.UpdatingTask) {
          ...
          const newKey = Object.keys(tasks).length.toString(); // Generate a unique key
          tasks[newKey] = { task, description, status };
      }
  ...
  document.getElementById('task-list').addEventListener('change', function (event) {
      if (event.target.type === 'checkbox') {
          ...
          const newStatus = event.target.checked ? 'finished' : 'open';
          tasks[key].status = newStatus;
          saveTasks(); // Save updated status to localStorage
          renderTasks();
      }
  });
